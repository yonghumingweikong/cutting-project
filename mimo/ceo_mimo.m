function [X_opt, y_opt, time, flag] = ceo_mimo(f, l, u, options, g, gamma, reference_c, reference_t, actual, Ts)
...
%
   ...
   while 1
      ...
      if (strcmpi(display, 'iter'))
         ...
         y_latest = mimo_rollout(X_opt(1),X_opt(2),X_opt(3),X_opt(4),X_opt(5),X_opt(6),X_opt(7),X_opt(8),reference_c,reference_t,Ts);
         clf; plot(y_latest); hold on; plot(actual,'--'); grid on; ylim([0,1.2]); pause(0.1); 
      end      
      if (t >= N)
